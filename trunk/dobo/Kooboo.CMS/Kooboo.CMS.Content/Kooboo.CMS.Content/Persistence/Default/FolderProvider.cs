...
namespace Kooboo.CMS.Content.Persistence.Default
{
        ...
        where T : Folder
    {
        ...
        private IEnumerable<T> GetFolders(Repository repository, string baseDir, Folder parent = null)
        {
            if (Directory.Exists(baseDir))
            {
                foreach (var item in IO.IOUtility.EnumerateDirectoriesExludeHidden(baseDir))
                {
                    ...
                    if (string.Compare(folderName, TextContentPath.ContentFolderName, true) != 0 && string.Compare(folderName, TextContentPath.ContentAttachementFolder, true) != 0)
                    {
        ...
        public void Export(Repository repository, IEnumerable<Folder> models, Stream outputStream)
        {
            var list = models;
        ...
        public void Import(Repository repository, string destDir, Stream zipStream, bool @override)
        {
            GetLocker().EnterWriteLock();
            try
            {
                ImportHelper.Import(destDir, zipStream, @override);
            }
            finally
            {
