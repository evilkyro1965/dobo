			...
			<tr class= "<%= index%2!=0? "even" : ""%> docTr" >
				<td>
					<input type="checkbox" name="Selected" class="select docs" id="<%= item.UUID %>" value="<%= item.UUID %>" />
						...
						<td><a class="f-icon document dialog-link " title="<%="Edit ".Localize() + "Parameters"%>" href="<%=this.Url.Action("Edit","TextContent",ViewContext.RequestContext.AllRouteValues().Merge("UserKey", (object)(item.UserKey)).Merge("UUID",(object)(item.UUID)))%>" ><%:item["Key"] ?? ""%></a></td>
		<td><%:item.Value ?? ""%></td>
