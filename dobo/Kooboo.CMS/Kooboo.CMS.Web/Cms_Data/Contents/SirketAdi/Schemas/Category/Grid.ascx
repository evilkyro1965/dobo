			...
			<tr class= "<%= index%2!=0? "even" : ""%> docTr" >
				<td>
					<input type="checkbox" name="Selected" class="select docs" id="<%= item.UUID %>" value="<%= item.UUID %>" />
						...
						<td><a class="f-icon document dialog-link " title="<%="Edit ".Localize() + "Category"%>" href="<%=this.Url.Action("Edit","TextContent",ViewContext.RequestContext.AllRouteValues().Merge("UserKey", (object)(item.UserKey)).Merge("UUID",(object)(item.UUID)))%>" ><%:item["Title"] ?? ""%></a></td>
...
<script language="javascript" type="text/javascript">
	kooboo.cms.content.textcontent.initGrid('<%="Are you sure you want to delect these items?".Localize()%>','<%="You have not select any item!"%>');
</script>
