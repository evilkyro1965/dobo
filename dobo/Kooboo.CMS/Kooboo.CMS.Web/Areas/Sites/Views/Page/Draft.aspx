    ...
    <div class="common-form" id="page-form">
        ...
        <p class="buttons">
            ...
            <%if (ServiceFactory.UserManager.Authorize(Kooboo.CMS.Sites.Models.Site.Current, ViewContext.HttpContext.User.Identity.Name, Kooboo.CMS.Account.Models.Permission.Sites_Page_PublishPermission))
              {%>
            <a href="<%=Url.Action("Publish",Request.RequestContext.AllRouteValues().Merge("FromDraft",true)) %>" name="Published" class="button dialog-link" title="<%:"Publish draft".Localize() %>">
                <%="Publish".Localize()%></a>
            <%} %>
            ...
            <%-- <a class="button" title="<%:"".Localize() %>" href="<%:returnUrl %>">
                <%="Return".Localize()%></a>--%>
        </p>
        <% } %>
    </div>
