...
<%var guid = Guid.NewGuid();
  ...
  var isFromFolder = !string.IsNullOrEmpty(Model.SelectionFolder);
%>
<div id="Column<%:Model.Name%>-form" class="Column<%:Model.Name%> ColumnForm  common-form">
    ...
    <div id="<%: Model.Name %>tabs" koobootab="koobooTab">
        ...
        <div class="tab-content" id="<%: Model.Name %>-tabSelectListItemsremove">
            <div class="clearfix">
                <p class="left">
                    <input type="radio" name="<%:preId  %>Manual" id="<%:preId %>-manual" <%if(!isFromFolder){%>
                        checked <%} %> value="Manual" />
                    <label for="<%:preId %>-manual" class="inline">
                        <%:"Manual".Localize() %></label>
                </p>
                <p>
                    <input type="radio" id="<%:preId %>-fromfolder" name="<%:preId  %>Manual" <%if(isFromFolder){%>
                        checked <%} %> value="FromFolder" />
            ...
            <%:Html.EditorFor(m => m.SelectionItems, new { id = preId + "SelectionItems" })%>
        </div>
