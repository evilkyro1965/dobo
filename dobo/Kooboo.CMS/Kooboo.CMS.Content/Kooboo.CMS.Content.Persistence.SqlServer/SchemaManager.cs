...
IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{0}')
    CREATE TABLE [{0}] (	
        ...
        protected static void ExecuteDDL(Repository repository, string ddl)
        {
            ...
            {
                SQLServerHelper.BatchExecuteNonQuery(repository.GetConnectionString(),
        ...
        {
            string sql = string.Format(@"SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = '{0}'", schema.Name);

            SqlCommand command = new SqlCommand() { CommandText = sql };

            return (int)SQLServerHelper.ExecuteScalar(schema.Repository.GetConnectionString(), command) != 0;
        }
