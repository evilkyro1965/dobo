...
namespace Kooboo.CMS.Content.Models
{
    ...
    {
        static string[] Sys_Fields = new string[] { 
            "Id",
            "UUID",
            "Repository",
            "FolderName",
    ...
    [DataContract]
    [Serializable]
    public class Schema : IRepositoryElement, IPersistable, ISchema
    {
        public Schema()
        { }
        ...
        {
            var summarizeField = this.columns.Where(it => it.Summarize == true).FirstOrDefault();
        ...
        public IEnumerable<Column> AllColumns
        {
            get
            {
                var sysColumns = new[] { Column.UUID, Column.UserKey, Column.UtcCreationDate, Column.Published, Column.OriginalUUID, Column.Id, Column.UserId, Column.Sequence };
                if (this.Columns == null)
                {
                    return sysColumns;
                }
