                ...
                yield return new ModelClientValidationRule()
                     {
                         ...
                         ErrorMessage = string.Format(SR.System_Web_Mvc_Resources.GetString("ClientDataTypeModelValidatorProvider_FieldMustBeNumeric"), column.Name)
                     };
            }
            if (column.DataType == DataType.String && column.Length > 0)
            {
                yield return new ModelClientValidationStringLengthRule(string.Format(SR.System_ComponentModel_DataAnnotations_Resources.GetString("StringLengthAttribute_ValidationError"), column.Name, column.Length)
                    , 0, column.Length);
            }
            if (!column.AllowNull)
            {
                yield return new ModelClientValidationRequiredRule(string.Format(SR.System_ComponentModel_DataAnnotations_Resources.GetString("RequiredAttribute_ValidationError"), column.Name));
            }
