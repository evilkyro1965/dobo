...
namespace Kooboo.CMS.Sites.Persistence.Caching
{
    public class PageProvider : CacheObjectProviderBase<Page>, IPageProvider
    {
        ...
        {
            string cacheKey = "GetDefaultPage";
            var defaultPage = site.ObjectCache().Get(cacheKey) as Page;
            if (defaultPage == null)
            {
                defaultPage = inner.GetDefaultPage(site);
                if (defaultPage != null)
                {
                    site.ObjectCache().Add(cacheKey, defaultPage, CacheProviderFactory.DefaultCacheItemPolicy);
                }
            }
            return defaultPage;
        }
        ...
        {
            inner.Import(site, destDir, zipStream, @override);
            site.ClearCache();
        }
