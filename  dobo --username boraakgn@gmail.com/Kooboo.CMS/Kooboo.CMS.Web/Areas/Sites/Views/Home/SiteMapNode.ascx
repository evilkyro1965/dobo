...
<% if (Model != null)
   {
       var isLocal = Model.Page.Site == Kooboo.CMS.Sites.Models.Site.Current;
   ...
   {  %>
        <ul>
            ...
            <li><a href="javascript:;" publishurl="<%=Url.Action("Publish","Page",ViewContext.RequestContext.AllRouteValues().Merge("FullName",Model.Page.FullName ).Merge("ReturnUrl", Request.RawUrl)) %>"
                unpublishurl="<%=Url.Action("OffLine","Page",ViewContext.RequestContext.AllRouteValues().Merge("FullName",Model.Page.FullName )) %>"
