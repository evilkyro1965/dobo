...
(function ($) {
    ...
    $.extend(options, __designer.design_js);
    ...
    */
    ...
    var createBtn = function (cls, title, click) {
        return $('<var class="btn ' + cls + '" title="' + title + '"></var>').click(click).hover(function () {
            $(this).addClass('btnhl');
        }, function () {
            $(this).removeClass('btnhl');
        });
    };
    ...
    content.prototype = {
        ...
        initialize: function () {
            ...
            var split = '<var class="sep"></var>';

            // btn move
            this.btnMove = $('<var class="movebtn" title="' + options.moveBtnTitle + '"></var>').appendTo(this.el);
            yardi.designCtx.disableSelection(this.btnMove);
            this.el.append(split);

            // btn edit
            this.btnEdit = createBtn('editbtn', options.editBtnTitle, function () {
                self.getPosition().editComponentDialog(self);
            }).appendTo(this.el);
            this.el.append(split);

            // btn remove
            this.btnRemove = createBtn('removebtn', options.removeBtnTitle, function () {
            ...
            // content name
            this.el.append('<var class="title" title="' + this.Name + '">' + this.Name + '</var>');
            this.btnTitle = $('.title', this.el);
            ...
            this.el.mousedown(function (ev) {
                self.canDrag = $(ev.target).hasClass('movebtn');
    ...
    */
    ...
    yardi.extend(htmlContent, content, {
        ...
        initialize: function () {
            ...
            this.htmlCon = $('<var class="htmlcon"></var>').appendTo(this.el);
            if (!this.isNew) {
                this.processHtml();
        ...
        detailStatus: function (show, animate) {
            ...
            this.btnZoom.removeClass('downbtn upbtn').addClass(show ? 'upbtn' : 'downbtn');
        ...
        processHtml: function () {
            ...
            // fix bug: set html to element innerHTML, whild style or script tag at the first position of html.
        ...
        processScript: function (con) {
            ...
            $.each(scripts, function () {
                if (!this.execuded) { exeScripts.push(this); }
            });
            if (!exeScripts.length) { return; }
            // process
            var write = document.write;
            var writeln = document.writeln;
            $.each(exeScripts, function () {
                var elem = this, p = elem.parentNode, next = elem.nextSibling;
                ...
                document.write = function (text) {
                    ...
                    for (var i = 0; i < temp.childNodes.length; i++) {
                        if (next) {
                            p.insertBefore(temp.childNodes[0], next);
                        } else {
                            p.appendChild(temp.childNodes[0]);
                        }
                    }
                };
